...
def compile_text():
    ...
    if len(target)<1:
        ...
        #помещаем в список статусов подсписки команд статусов 
        k=0
        ...
        for i in range(0,k):
            for j in range(0,len(lines)):
                if "q"+"{id}".format(id=i)+" " in lines[j][:int(len(lines[j])/2)]:
                    #stats[i].append(lines[j].lstrip("q"+"{id}".format(id=i) ))
